from models import AnalysisBreakdown, ResourceType, ResourceReference


class ResourcesAnalyzer:

    def _create_breakdown(self, resources_list) -> dict[ResourceType, set[ResourceReference]]:
        breakdown = {}
        for resource_type in ResourceType:
            resources_of_type = set([r for r in resources_list if r.type == resource_type])
    ...
    def analyze(self, name, client_used_refs, lib_used_refs, lib_packaged_res) -> AnalysisBreakdown:
        ...
        return AnalysisBreakdown(
            project_name=name,
            used_resources=self._create_breakdown(lib_used_refs),
            unused_resources=self._create_breakdown(lib_unused_refs),
            unused_size_bytes=sum([r.size for r in lib_unused_packaged_res])
        )
